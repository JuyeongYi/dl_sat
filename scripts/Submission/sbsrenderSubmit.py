...
def __main__(*args):
    global g_input, g_dialog, g_inputGraph, g_usageOn, g_output, g_outBitDepth, g_outName
    # Tab Start
    ...
    g_input = g_dialog.AddSelectionControlToGrid(INPUT, "FileBrowserControl", "", "Substance Archaive File (*.sbsar)", r, c := c + 1, colSpan=2)
    ...
    g_inputGraph.currentTextChanged.connect(GraphChanged)

    g_dialog.AddControlToGrid(f"{OUTPUT}Label", "LabelControl", "Outputs to export", r := r + 1, c := 0)
    g_output = g_dialog.AddControlToGrid(OUTPUT, "TextControl", "", r, c := c + 1)
    g_usageOn = g_dialog.AddSelectionControlToGrid("outputBasedUsage", "CheckBoxControl", False, "Output Based Usage", r := r, c := c + 1)
    ...
    g_dialog.AddSelectionControlToGrid(PARM_CSV, "FileBrowserControl", "", "Comma Seperated Value (*.csv)", r, c := c + 1, colSpan=2)

    g_dialog.AddControlToGrid(f"{OUT_DEPTH}Label", "LabelControl", "Output Bit Depth", r := r + 1, c := 0, expand=False)
    g_outBitDepth = g_dialog.AddComboControlToGrid(OUT_DEPTH, "ComboControl", "16", ("8", "16", "16f", "32f"), r, c := c + 1)

    g_dialog.AddControlToGrid(f"{OUTPUT_PATH}Label", "LabelControl", "Destination Path", r := r + 1, c := 0, expand=False)
    g_dialog.AddSelectionControlToGrid(OUTPUT_PATH, "FolderBrowserControl", "", "", r, c := c + 1, colSpan=4)

    g_dialog.AddControlToGrid(f"{OUTPUT_NAME}Label", "LabelControl", "File Name Pattern", r := r + 1, c := 0, expand=False)
    g_outName = g_dialog.AddControlToGrid(OUTPUT_NAME, "TextControl", "{inputGraphUrl}_{outputNodeName}", r, c := c + 1, colSpan=3, expand=True)
    g_dialog.AddComboControlToGrid(OUTPUT_FORMAT, "ComboControl", "png",
                                   ("surface", "dds", "bmp", "jpg", "jif", "jpeg", "jpe", "png", "tga", "targa", "tif", "tiff", "wap", "wbmp", "wbm", "psd", "psb", "hdr", "exr", "webp"), r,
                                   c := 4)

    g_dialog.AddControlToGrid("outputsizeLabel", "LabelControl", "Output Size(x, y)", r := r + 1, c := 0, expand=False)
    g_dialog.AddRangeControlToGrid("outputsizeX", "SliderControl", 10, 0, 13, 0, 1, r, c := c + 1, colSpan=2)
    g_dialog.AddRangeControlToGrid("outputsizeY", "SliderControl", 10, 0, 13, 0, 1, r, c := c + 2, colSpan=2)
...
def UsageToggled(toggled):
    ...
    try:
        ...
        if not os.path.exists(g_input.TheValue):
            return
        ...
        assert "OUTPUT" in gDict, f"No output in {currGraph}"

        USAGE_FAIL = ("NO_USAGE", "MULTIPLE_USAGE")
        usageOn = toggled
        outputDict = gDict["OUTPUT"]
        ...
        if usageOn:
            ...
            for k in outputDict.keys():
                outputNames.append(k)
            txt = ' '.join(outputNames)
            g_output.setText(txt)
...
def GraphChanged(selectedGraph):
...
def OnSBSARInputChanged(args):
    ...
    g_inputGraph.clear()
    inputGraphs = tuple(g_sbsarDict.keys())
    for k in inputGraphs:
        g_inputGraph.addItem(k)
